...
export default function Home() {
    ...
    return (
        <div className="bg-white min-h-screen">
            ...
            {/* 2. HERO SECTION */}
            <div className="relative bg-gradient-to-br from-primary via-[#6D2822] to-secondary pb-20 pt-10 sm:pb-24 lg:pb-32 overflow-hidden">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                    <div className="text-center mb-10">
                        <h1 className="text-4xl tracking-tight font-extrabold text-white sm:text-5xl md:text-6xl mb-4">
                            Your pick of rides at <span className="text-primary-light">low prices</span>
                        </h1>
                        <p className="mt-3 max-w-md mx-auto text-base text-red-50 opacity-90 sm:text-lg md:mt-5 md:text-xl md:max-w-3xl">
                            Travel responsibly and comfortably. The perfect ride is just a search away.
                        </p>
                    </div>

                    {/* 3. MAIN SEARCH BAR (Card Style) */}
                    <div className="bg-white rounded-2xl shadow-xl p-4 md:p-2 max-w-5xl mx-auto transform translate-y-4 border border-gray-100">
                        <form onSubmit={handleSearch} className="flex flex-col md:flex-row items-center gap-2">
                            ...
                            <div className="relative w-full md:w-1/4 group">
                                ...
                                <input
                                    ...
                                    className="block w-full pl-10 pr-3 py-4 border-none rounded-xl bg-gray-50 hover:bg-gray-100 focus:ring-2 focus:ring-primary focus:bg-white transition-colors text-gray-900 placeholder-gray-500"
                                    value={from}
            ...
            <div className="bg-background-light py-20 overflow-hidden">
                ...
                <div className="relative w-full overflow-hidden group">
                    <div className="flex animate-scroll gap-6 w-[200%] hover:pause">
                        {[...quotes, ...quotes, ...quotes].map((quote, i) => (
                            <div key={i} className="flex-shrink-0 w-96 bg-white p-8 rounded-2xl shadow-md border-l-4 border-primary transform transition-transform hover:-translate-y-1">
                                <p className="text-gray-700 italic mb-6 text-lg">"{quote.text}"</p>
                                <div className="flex items-center">
                                    <div className="h-12 w-12 rounded-full bg-gradient-to-br from-primary to-secondary flex items-center justify-center text-white font-bold text-xl shadow-sm">
                                        {quote.author.charAt(0)}
                                    </div>
                                    <div className="ml-4 text-left">
                                        <p className="text-base font-bold text-gray-900">{quote.author}</p>
                                        <p className="text-sm text-primary font-medium">{quote.role}</p>
                                    </div>
                                </div>
                            </div>
                        ))}
